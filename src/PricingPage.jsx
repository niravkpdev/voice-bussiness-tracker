...
import { Check, X, Shield, Lock, LifeBuoy, Zap, Crown } from 'lucide-react';
...
export function PricingPage({ onClose, onUpgrade, onSelectPlan, onContactSales, isLoggedIn }) {
  ...
  return (
    <div className="pricing-page-container fade-in" style={{
      ...
      overflowY: 'auto', display: 'flex', flexDirection: 'column', color: 'var(--text-primary)'
    }}>
      {toast && (
        <div style={{
          position: 'fixed', top: '20px', left: '50%', transform: 'translateX(-50%)',
          background: 'var(--text-primary)', color: 'var(--bg-primary)', padding: '12px 24px',
          borderRadius: '8px', zIndex: 1100, fontWeight: 600, boxShadow: '0 4px 16px rgba(0,0,0,0.15)'
        }}>
          {toast}
        </div>
      )}
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <span className="brand-title" style={{ fontWeight: 700, fontSize: '1.25rem', color: 'var(--text-primary)' }}>
            Trinetr Business Suite
          </span>
        </div>
        {onClose && (
          <button 
            className="icon-button pricing-close-button" 
            onClick={onClose}
            aria-label="Close"
            type="button"
            style={{
      ...
      <section style={{ textAlign: 'center', padding: '50px 20px 30px', maxWidth: '800px', margin: '0 auto' }}>
        ...
        <div style={{
          ...
          >
            Yearly <span style={{ color: '#10b981', fontSize: '0.8rem', marginLeft: '4px', fontWeight: 700 }}>Save 20%</span>
          </button>
        </div>
      </section>
      ...
      <section style={{
        ...
        <div style={{
          ...
          <button className="saas-primary-button full" style={{ marginBottom: '32px' }} onClick={() => handleAction('Professional')}>
        ...
        <div style={{
          ...
          <h3 style={{ fontSize: '1.5rem', marginBottom: '8px', fontWeight: 700, color: 'var(--text-primary)' }}>Enterprise</h3>
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginBottom: '24px', minHeight: '40px', lineHeight: 1.5 }}>
            Custom limits and dedicated management.
          </p>
          <div style={{ marginBottom: '24px', display: 'flex', alignItems: 'baseline', height: '48px' }}>
            <span className="pricing-page-price" style={{ fontSize: '2.5rem', fontWeight: 800, color: 'var(--text-primary)' }}>
              Custom
            </span>
          </div>
        ...
        <div style={{ maxWidth: '1000px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '36px' }}>
          <div>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', lineHeight: 1.6, margin: 0 }}>
              All transactions are encrypted and processed securely by industry-leading payment gateways.
            </p>
          </div>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px', fontWeight: 700, color: 'var(--text-primary)', fontSize: '1.05rem' }}>
              <Shield size={20} color="#10b981" /> Your Data is Yours
            </div>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', lineHeight: 1.6, margin: 0 }}>
              We never sell your data. You can export your full business history as a JSON backup at any time.
            </p>
          </div>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px', fontWeight: 700, color: 'var(--text-primary)', fontSize: '1.05rem' }}>
              <LifeBuoy size={20} color="#ef4444" /> Cancel Anytime
            </div>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', lineHeight: 1.6, margin: 0 }}>
              No long-term contracts or hidden fees. If you downgrade, you keep all your existing data.
            </p>
          </div>
        </div>
      </section>
